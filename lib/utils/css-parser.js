...
class CSSClassExtractor {
  constructor(options = {}) {
    ...
    // Use LRU cache with configurable size limit (default 50 entries)
  ...
  extractClassesFromSelector(selector) {
    ...
    try {
      ...
      // Log error in debug mode only, continue processing
  ...
  extractClassesFromCSS(cssContent, filePath) {
    ...
    try {
      ...
      // Log error and return empty array for malformed CSS
      if (process.env.DEBUG) {
  ...
  getAllDefinedClasses() {
    ...
    cssFiles.forEach(filePath => {
      try {
        ...
        // Log error in debug mode only, skip this file
        if (process.env.DEBUG) {
