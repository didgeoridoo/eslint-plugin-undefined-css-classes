...
module.exports = {
  ...
  create(context) {
    ...
    // Resolve to absolute path and ensure it's within project boundaries
    baseDir = path.resolve(projectRoot, baseDir);
    if (!baseDir.startsWith(projectRoot)) {
      throw new Error(`baseDir must be within project directory. Got: ${baseDir}, Project: ${projectRoot}`);
    }
