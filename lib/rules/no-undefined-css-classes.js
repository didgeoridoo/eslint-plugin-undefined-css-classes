...
module.exports = {
  ...
  create(context) {
    ...
    // Debug logging
    if (config.debug) {
      console.log('[MAIN RULE] no-undefined-css-classes rule instantiated');
      console.log('[MAIN RULE] Config:', JSON.stringify(config, null, 2));
      console.log('[MAIN RULE] Context filename:', context.filename || context.getFilename?.());
    }
    ...
    return {
      ...
      SvelteAttribute(node) {
        ...
        if (config.debug) {
          console.log('[MAIN RULE DEBUG] SvelteAttribute handler called');
        }
        ...
        if (isClassAttribute) {
          ...
          if (config.debug) {
            console.log('[MAIN RULE DEBUG] Found class attribute, extracted classes:', classes);
            const defined = getDefinedClasses();
            console.log('[MAIN RULE DEBUG] Defined CSS classes count:', defined.size);
            console.log('[MAIN RULE DEBUG] First 10 defined classes:', Array.from(defined).slice(0, 10));
          }
