...
const SvelteStyleParser = require('../utils/svelte-style-parser');
const {
...
module.exports = {
  ...
  create(context) {
    ...
    const svelteStyleParser = new SvelteStyleParser();

    let definedClasses = null;
    let ignoreRegexes = null;
    let svelteStyleClasses = null;

    const getDefinedClasses = () => {
      if (definedClasses === null) {
        definedClasses = new Set(cssExtractor.getAllDefinedClasses());
        
        // If this is a Svelte file, also include classes from <style> blocks
        const filename = context.getFilename ? context.getFilename() : context.filename;
        if (filename && filename.endsWith('.svelte')) {
          const sourceCode = context.getSourceCode ? context.getSourceCode().text : context.sourceCode.text;
          if (sourceCode) {
            const styleClasses = getSvelteStyleClasses(sourceCode);
            styleClasses.forEach(cls => definedClasses.add(cls));
          }
        }
      }
    ...
    const getSvelteStyleClasses = (sourceCode) => {
      if (svelteStyleClasses === null) {
        svelteStyleClasses = svelteStyleParser.extractStyleBlockClasses(sourceCode);
      }
      return svelteStyleClasses;
    };
